<?php
	...
	$select = array(0=>'semua',1=>'Lunas','baru'=>'Baru','pending'=>'Pending','edit'=>'Edit','batal'=>'Batal');
	// echo $id;
?>
					...
					<table class="table table-bordered table-striped table-mailcard" id="jsonuser">
						<thead>
							<tr>
								...
								<th style="width:10% !important;">No. Transaksi</th>
								<th>Tgl. Transaksi</th>
								<th>Konsumen</th>
								<th>No. HP</th>
								<th>Kasir</th>
						...
						<tbody>
							<?php if(!empty($posts)){
								...
								<tr>
									<td><?php echo $no; ?></td>
									<td><a href="#" data-toggle="modal" data-id="<?php echo $row["id_invoice"]; ?>" data-modEdit="edit" data-target="#OpenCart-1" id="cart">#<?php echo $row["id_invoice"]; ?></a></td>
...
<script>
	...
	function searchFilter(page_num){
		...
		var tgl = $('#tgl').val();
		var trx = $('#trx').val();
		var urlnya = '<?php echo base_url("penjualan/ajaxPaginationData/"); ?>'+page_num
		$.ajax({
			type: 'POST',
			url: urlnya,
